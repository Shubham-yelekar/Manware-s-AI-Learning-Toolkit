...
> **Looking for a different AI agent?** This repo has separate branches for each supported tool. Switch to the branch matching your agent — `Cursor`, `cursor`, `claude-code`, `opencode`, or `antigravity` — to get the right configuration and code.
...
2. **Open Cursor Chat** in your editor.
3. **Type a slash command** like `/hint`, `/debug`, `/explain`, etc.
4. **Answer the questions** Cursor asks you. Don't skip them — that's where the learning happens.
5. **Write the code yourself.** AI will guide, not write.

> **Important!:** Disable Cursor inline completions while learning. Use Chat mode only.

---

## The 12 Prompts

Each prompt is a workflow. Invoke it with a `/command` in Cursor.
...
```
.cursorrules                   # Global learning philosophy
.cursor/
├── rules/                     # 12 user-facing workflows (slash commands)
...
- **Cursor** (Chat mode) in your editor
...
- **Skills** require a Cursor plan that supports custom skills. Prompts work everywhere.
...
- This is a prompt-based toolkit, not an app. It works through Cursor conversations.
